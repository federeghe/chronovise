...
class KS_Test_Test : public ::testing::Test {
protected:


};

TEST_F(KS_Test_Test, GetterSetter) {
	TestKS<unsigned long, unsigned long> ks(0.05, distribution_t::EVT_GEV);

	EXPECT_EQ(0.05, ks.get_significance_level());
	EXPECT_EQ(false, ks.is_reject());
	EXPECT_EQ(true, ks.has_power());

}

TEST_F(KS_Test_Test, DistributionACCEPT) {
	...
	ks.set_ref_distribution(du_ref_shared);
...
, 1.4324, 1.8253, 1.0835, 1.1332
	...
	EXPECT_FALSE(ks.is_reject());

}

TEST_F(KS_Test_Test, DistributionREJECT) {
